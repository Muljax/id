...
const PRINCIPAL_PERMISSION_REGEX =
	/^ssh:principal:([a-zA-Z0-9_.][a-zA-Z0-9_.-]{0,63})$/;

export async function ensureCustomPermissionsExist(
	db: Database,
	permissionIds: string[],
): Promise<void> {
	const now = Date.now();
	const customToInsert: (typeof permissions.$inferInsert)[] = [];

	for (const permId of permissionIds) {
		const principalMatch = permId.match(PRINCIPAL_PERMISSION_REGEX);
		if (principalMatch) {
			const principalName = principalMatch[1];
			if (principalName !== "*") {
				customToInsert.push({
					id: permId,
					name: `SSH Principal: ${principalName}`,
					description: `Claim the UNIX principal '${principalName}' on issued certificates`,
					resource: "ssh",
					isSystem: false,
					createdAt: now,
					updatedAt: now,
...
export async function createRole(
	...
	if (input.permissions && input.permissions.length > 0) {
		...
		await ensureCustomPermissionsExist(db, uniquePerms);
