...
export async function cleanupExpiredSessions(db: Database) {
	const result = (await db
		.delete(sessions)
		.where(lte(sessions.expiresAt, Date.now()))) as unknown as {
		meta?: { changes: number };
		changes?: number;
	};
...
/**
 * Elevates a session for sensitive operations for a specified duration.
 *
 * @param db The database connection.
 * @param sessionId The ID of the session to elevate.
 * @param durationMs Duration in milliseconds (default: 5 minutes).
 * @returns The epoch timestamp (ms) until which the session is elevated.
 */
export async function elevateSession(
	...
	const elevatedUntil = Date.now() + durationMs;
	await db
		.update(sessions)
		.set({
			elevatedUntil,
