...
describe("Auth Middleware & Scope Attenuation Protection", () => {
	test("requireStrictSessionAuth allows interactive cookie sessions", async () => {
		...
		app.post("/account/password", requireStrictSessionAuth, (c) =>
			c.json({ ok: true }),
		);

		const res = await app.request("/account/password", { method: "POST" });
		expect(res.status).toBe(200);
		expect(await res.json()).toEqual({ ok: true });
	});

	test("requireStrictSessionAuth strictly rejects OAuth bearer tokens even with users:write or wildcard scopes", async () => {
		const app = new Hono<AppEnv>();
		app.use("*", async (c, next) => {
			c.set("user", {
				id: "user-1",
		...
		expect(body.message).toContain("interactive user session");
	});
