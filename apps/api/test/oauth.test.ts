...
describe("OAuth 2.0 Device Authorization Grant (RFC 8628)", () => {
	test("generates valid human-friendly user codes", async () => {
		const { generateUserCode, normalizeUserCode } = await import(
			"../src/lib/oauth/device"
		);
		...
		// Normalization tests
		expect(normalizeUserCode("wdjb4921")).toBe("WDJB-4921");
		expect(normalizeUserCode("wdjb-4921")).toBe("WDJB-4921");
		expect(normalizeUserCode(" WDJB - 4921 ")).toBe("WDJB-4921");
	});

	test("OpenID Discovery includes device authorization endpoint and grant type", async () => {
		const openidConfigRoute = (
			await import("../src/routes/well-known/openid-configuration/get")
		).default;
		const { Hono } = await import("hono");
		...
		const res = await app.request("/.well-known/openid-configuration", {}, {
			OIDC_ISSUER: "https://auth.muljax.com",
		} as Env);

		expect(res.status).toBe(200);
		const json = (await res.json()) as Record<string, unknown>;
		expect(json.device_authorization_endpoint).toBe(
			"https://auth.muljax.com/oauth/device/code",
		);
		expect(
			(json.grant_types_supported as string[]).includes(
				"urn:ietf:params:oauth:grant-type:device_code",
			),
		).toBe(true);
	});

	test("Device endpoints handle CORS preflight and requests from dashboard origin", async () => {
		const { app } = await import("../src/app");
		const env = {
			DASHBOARD_DOMAIN: "id.muljax.dev",
			LOCALHOST: "false",
		} as unknown as Env;

		// Preflight OPTIONS check
		const optionsRes = await app.request(
			"/oauth/device/details?user_code=WDJB-4921",
			{
				method: "OPTIONS",
				headers: {
		...
		expect(optionsRes.headers.get("Access-Control-Allow-Credentials")).toBe(
			"true",
		);
	});
});
