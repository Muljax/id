...
function UsersPage() {
	...
	const [sessionsTargetUser, setSessionsTargetUser] =
		useState<AdminUser | null>(null);
	...
	return (
		<div className="space-y-8 max-w-4xl">
					...
					<div className="divide-y divide-white/6">
						{users.map((user) => (
							<UserRow
								...
								onManageSessions={(target) => setSessionsTargetUser(target)}
								onDeleteUser={(target) => setDeleteTargetUser(target)}
							/>
						))}
					</div>
				</Card>
			...
			{sessionsTargetUser && (
				<UserSessionsModal
					open={Boolean(sessionsTargetUser)}
					user={sessionsTargetUser}
					isSelf={currentUser?.id === sessionsTargetUser.id}
					onClose={() => setSessionsTargetUser(null)}
					onManageLifecycle={(target) => {
						setSessionsTargetUser(null);
						setLifecycleTargetUser(target);
					}}
				/>
			)}
