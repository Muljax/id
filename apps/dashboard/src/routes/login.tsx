...
function LoginPage() {
	...
	const finishLogin = useCallback(
		...
		[setUser, refresh, return_to, forceLogin, navigate],
	);
	...
	useEffect(() => {
		...
		async function initPasskeyAutofill() {
			if (user || forceLogin || isClosed) return;
			...
			try {
				...
				const res = await verifyPasskeyLogin(
					response,
					challengeId,
					admin_key?.trim() || undefined,
				);
				await finishLogin(res.user);
			} catch {
				// Silently handle cancelled or ignored autofill mediation
			}
		}

		void initPasskeyAutofill();

		return () => {
			isMounted = false;
		};
	}, [user, forceLogin, isClosed, admin_key, finishLogin]);
