---
title: Host Revocation (KRL) Sync
description: Synchronizing revoked certificate serial numbers to target servers using systemd timers.
---

import { Tabs, TabItem, Steps } from '@astrojs/starlight/components';

OpenSSH servers check incoming user certificates against a local Key Revocation List (KRL) configured via the `RevokedKeys` directive in `/etc/ssh/sshd_config`.

Muljax ID provides an automated endpoint that formats revoked certificates into the line-separated text syntax expected by OpenSSH:

```http
GET /api/ssh/ca/revoked-keys?format=raw
```

```text
serial: 117284521030851699
serial: 117284521030851700
```

---

## Deployment Options

<Tabs>
  <TabItem label="Automated Setup Script">

Run this complete setup block as `root` on your target server (replace `https://api.example.com` with your API domain):

```sh
# 1. Create systemd service
sudo tee /etc/systemd/system/ssh-revoked-keys.service > /dev/null << 'EOF'
[Unit]
Description=Sync OpenSSH Revoked Keys
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/sh -c 'curl -fsSL "https://api.example.com/api/ssh/ca/revoked-keys?format=raw" -o /etc/ssh/revoked_keys.tmp && chmod 644 /etc/ssh/revoked_keys.tmp && mv /etc/ssh/revoked_keys.tmp /etc/ssh/revoked_keys'
EOF

# 2. Create systemd timer (runs every 15 minutes)
sudo tee /etc/systemd/system/ssh-revoked-keys.timer > /dev/null << 'EOF'
[Unit]
Description=Sync OpenSSH Revoked Keys Periodically

[Timer]
OnBootSec=1min
OnUnitActiveSec=15min
Persistent=true

[Install]
WantedBy=timers.target
EOF

# 3. Configure sshd and enable timer
sudo touch /etc/ssh/revoked_keys
grep -qxF 'RevokedKeys /etc/ssh/revoked_keys' /etc/ssh/sshd_config || echo 'RevokedKeys /etc/ssh/revoked_keys' | sudo tee -a /etc/ssh/sshd_config
sudo systemctl daemon-reload
sudo systemctl enable --now ssh-revoked-keys.timer
sudo systemctl reload sshd
```

  </TabItem>
  <TabItem label="Service Unit">

File: `/etc/systemd/system/ssh-revoked-keys.service`

```ini
[Unit]
Description=Sync OpenSSH Revoked Keys
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/sh -c 'curl -fsSL "https://api.example.com/api/ssh/ca/revoked-keys?format=raw" -o /etc/ssh/revoked_keys.tmp && chmod 644 /etc/ssh/revoked_keys.tmp && mv /etc/ssh/revoked_keys.tmp /etc/ssh/revoked_keys'
```

  </TabItem>
  <TabItem label="Timer Unit">

File: `/etc/systemd/system/ssh-revoked-keys.timer`

```ini
[Unit]
Description=Sync OpenSSH Revoked Keys Periodically

[Timer]
OnBootSec=1min
OnUnitActiveSec=15min
Persistent=true

[Install]
WantedBy=timers.target
```

  </TabItem>
</Tabs>

---

## Verifying the Host Setup

<Steps>

1. **Check Timer Execution**:
   ```sh
   systemctl list-timers ssh-revoked-keys.timer
   ```
   Ensure the timer shows active triggers for future executions.

2. **Trigger Manual Synchronization**:
   ```sh
   sudo systemctl start ssh-revoked-keys.service
   ```

3. **Inspect Revoked Keys Content**:
   ```sh
   cat /etc/ssh/revoked_keys
   ```

4. **Verify Service Logs**:
   ```sh
   journalctl -u ssh-revoked-keys.service -n 50
   ```

</Steps>

:::note[OpenSSH Immediate Rejection]
When a client connects with a certificate whose serial number matches an entry in `/etc/ssh/revoked_keys`, OpenSSH aborts authentication before password or key fallback:
```text
debug1: Certificate revoked by file /etc/ssh/revoked_keys
Connection closed by authenticating user alice
```
:::
