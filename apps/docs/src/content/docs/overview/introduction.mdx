...
<CardGrid>
  ...
  <Card title="Strict Ed25519 Cryptography" icon="padlock">
...
```mermaid title="Muljax ID System & Edge Topology"
flowchart TD
...
<Callout type="tip" title="Edge Native & Zero Cold Starts">
Every component of Muljax ID runs at the Cloudflare Edge within isolated V8 workers, achieving sub-millisecond p95 latency worldwide with zero dedicated database servers to manage.
</Callout>
...
- **`apps/docs`**: Interactive documentation portal featuring live schema reflection and OpenAPI endpoint explorers.
