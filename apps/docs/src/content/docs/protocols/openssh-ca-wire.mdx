...
Muljax ID implements a native, zero-dependency OpenSSH certificate binary serialization and deserialization engine in `@id/crypto/ssh` (`packages/crypto/src/ssh/`). This document is an exhaustive engineering deep dive into the exact binary wire layout, endianness, cryptographic signing boundaries, and host verification pipelines conforming strictly to OpenSSH's certificate specification ([IETF draft-miller-ssh-cert](https://datatracker.ietf.org/doc/draft-miller-ssh-cert/)) and RFC 4251.

---

## 1. Governing Protocol Standards

The OpenSSH certificate implementation adheres strictly to four foundational standards:
...
4. **[PROTOCOL.krl](https://cvsweb.openbsd.org/annotate/src/usr.bin/ssh/PROTOCOL.krl?rev=HEAD)**: OpenSSH Key Revocation List (KRL) binary format specification. Defines the wire format for KRL sections, including revoked keys, certificates, key IDs, and serial-number ranges.
...
All primitive data types are serialized in **network byte order (big-endian)**. The implementation in `@id/crypto/ssh` (`packages/crypto/src/ssh/wire.ts`) uses contiguous `Uint8Array` allocations and `DataView` operations without any intermediate string conversions:
...
```ts
// @id/crypto/ssh (packages/crypto/src/ssh/wire.ts)
...
```mermaid title="OpenSSH Binary Certificate Envelope Layout"
flowchart TD
...
### Field 10: Critical Options Wire Buffer (`criticalOptions`)
OpenSSH certificates enforce strict requirements on critical options:
1. **Lexicographical Ordering**: Options must be sorted strictly by name in ascending ASCII order (e.g. `force-command` before `source-address`).
2. **Inner String Envelope**: String values (such as allowed CIDR ranges for `source-address` or shell commands for `force-command`) must be wrapped inside an inner string wire format (`new SSHWriter().writeString(val).toUint8Array()`), written to the outer buffer with `writeBytes(innerWire)`.

```ts
// apps/api/src/lib/ssh/certificate.ts
const criticalOptionsWriter = new SSHWriter();
const sortedOptions = Object.entries(data.criticalOptions ?? {}).sort(([a], [b]) =>
    a.localeCompare(b),
);

for (const [name, val] of sortedOptions) {
    criticalOptionsWriter.writeString(name);
    const innerWriter = new SSHWriter();
    innerWriter.writeString(val);
    criticalOptionsWriter.writeBytes(innerWriter.toUint8Array());
}
writer.writeBytes(criticalOptionsWriter.toUint8Array());
```
...
```mermaid title="Target Host OpenSSH Certificate Verification Ceremony"
sequenceDiagram
    autonumber
...
OpenSSH's native binary KRL format provides maximum space efficiency when revoking certificates by serial number across host fleets. Muljax ID builds binary KRL payloads conforming directly to OpenSSH `PROTOCOL.krl` via `@id/crypto/ssh` (`packages/crypto/src/ssh/krl.ts`).
