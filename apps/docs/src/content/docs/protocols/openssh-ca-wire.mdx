...
description: RFC 4251 and OpenSSH CERT01 binary wire format implementation details verified against apps/api/src/lib/ssh.
---

`ID` implements a native, zero-dependency OpenSSH certificate binary serialization and deserialization engine in `apps/api/src/lib/ssh/`. This document is an exhaustive engineering deep dive into the exact binary wire layout, endianness, cryptographic signing boundaries, and host verification pipelines conforming strictly to OpenSSH's `PROTOCOL.certkeys` specification and RFC 4251.
...
2. **[OpenSSH PROTOCOL.certkeys](https://github.com/openssh/openssh-portable/blob/master/PROTOCOL.certkeys)**: OpenSSH certificate format version 1 (`CERT01`), covering user/host key certificates, critical options, extensions, and signature wrappers.
...
`ID` mints certificates with the five standard interactive extensions enabled by default:
