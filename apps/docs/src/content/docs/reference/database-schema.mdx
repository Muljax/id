...
`ID` utilizes [Cloudflare D1](https://developers.cloudflare.com/d1/) (distributed edge SQLite) managed via [Drizzle ORM](https://orm.drizzle.team). Foreign key constraints and cascading deletes (`ON DELETE CASCADE`) are strictly enabled across all relational boundaries.
