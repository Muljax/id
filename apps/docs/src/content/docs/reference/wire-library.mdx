...
description: Developer API reference for @id/crypto/ssh binary wire serialization, parsing, and certificate generation.
---

import { Badge } from '@/components/mdx';

The `@id/crypto/ssh` package (`packages/crypto/src/ssh/`) provides a zero-dependency TypeScript implementation of RFC 4251 binary serialization, OpenSSH public key parsing, Ed25519 user certificate minting, and strict POSIX sanitization.

---

## Architecture Overview

```mermaid title="SSH Wire Module Architecture"
flowchart LR
...
## 3. Certificate Functions (`@id/crypto/ssh`)
...
## 4. Key Management Functions (`@id/crypto/ssh`)
...
```ts
const hashBuffer = await crypto.subtle.digest("SHA-256", wireBytes as BufferSource);
...
## 5. Security Validation (`@id/crypto/ssh`)
