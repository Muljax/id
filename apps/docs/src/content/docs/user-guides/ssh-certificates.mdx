...
Traditional SSH keys require manually distributing public keys into every destination host's `~/.ssh/authorized_keys`. With the `ID` Certificate Authority, your public key is signed by the CA into a temporary **certificate**, granting instant access to all hosts that trust the CA.
